...
def uuid_get(date):
    ...
    if date == '20160717':
        sql = "select uuid from combine where date='%s';"%date###若为第一天则为全部用户，往后则为新增用�?
    else:
        sql = "select uuid from combine where date='%s' and uuid not in (select uuid from combine where date>='20160717' and date<'%s')"%(date,date)
...
def uuid_dict_process(date,uuid_dict):
    ...
    return uuid_info
###建立用户�?
def new_user_info_create():
    try:
        conn=MySQLdb.connect(host='10.100.3.64',user='hefang',passwd='NYH#dfjirD872C$d&ss',port=3306,charset='utf8')
        cur=conn.cursor()
        conn.select_db('live_user')
        cur.execute('drop table if exists new_user;create table new_user (`date` int(8),`uuid` varchar(50),birthday varchar(50),sex varchar(10),province varchar(50),'\
                    'city varchar(50),nickname varchar(50),mobile varchar(20),isVip varchar(10),vipExpiretime varchar(50));')
        cur.execute('alter table new_user add index new_user_index (`date`);')
        conn.commit()
        cur.close
        conn.close()
    except:
        pass
###将数据插入用户表
def new_user_info_insert(conn,cur,date,uuid_insert):
    error_path = '/root/hf/live_user/new_user'  # 存放插入错误的日志信�?
    if not os.path.exists(error_path):
        os.mkdir(error_path)
    insert_tag = 1  ###插入成功表示1
    try:
        sql = 'insert into new_user values('+','.join(map(lambda o: "%s",range(0,10)))+')'
        cur.executemany(sql,uuid_insert)
        conn.commit()
    except MySQLdb.Error,e:
        insert_tag = 0  ###插入失败表示0
        error_insertlog_path = '/root/hf/live_user/new_user/new_user_error_' + date + '.txt'  # 存放插入错误的日志信�?
...
def day_get(d):
    ...
    return date_end
##更新new_user中的birthday，将1970-01-01设置为空
def  update_new_user_birthday(conn,cur,date):
    try:
        cur.execute("update new_user set birthday=null where birthday='1970-01-01' and date=%s;" % date)
        conn.commit()
        print "update new_user birthday success"
    except:
        print "update new_user birthday failure"

if __name__ == '__main__':
    start_date = datetime.date(2016,7,17)  ###数据统计开始时�?
    # 获取当前时间
    d = datetime.datetime.now()
    end_date_f = day_get(d)  ##当前日期的前一�?
    date_list = datelist_new(start_date,end_date_f)  # 获得日期列表
    # 循环生成每天的new_user数据时间
    for date_loc in range(27,30):
        date = date_list[date_loc]
        uuid_data = uuid_get(date)###获取新增用户uuid
        length = len(uuid_data)
        if length > 0:
            try:
                if date == '20160717':
                    new_user_info_create()
                else:
                    pass
                ...
                for i in tqdm(range(length_split)):
                    ...
                    insert_tag = new_user_info_insert(conn,cur,date,uuid_insert)
                    if insert_tag == 1:
                        insert_success += len(uuid_list)  # 记录成功插入的条�?
                    else:
                        pass
                ##更新birthday
                update_new_user_birthday(conn, cur, date)
                cur.close
                conn.close
                print 'insert %s daily new user info into database success'%date
            except:
                print 'insert %s daily new user info into database fail'%date