...
if __name__ == '__main__':
    ...
    # 循环生成每天的date_combine数据时间
    for date_loc in range(40,41):
            date_info = date_list[ date_loc ].split(',')
            end_date = datetime.date(int(date_info[ 0 ]), int(date_info[ 1 ]), int(date_info[ 2 ]))  ###本次数据插入时间
            #end_date = datetime.date(2016,7,18)###数据汇总插入时�?
            ...
            if len(data_all) > 0:
                ...
                try:
                    if len(data_all) > 0:
                        ...
                        length_split = length/length_list+1###将数据�?�段，每一万行�?一�?
