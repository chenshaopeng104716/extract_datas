...
"""
@author:csp
@function: 获取type=1时vid，对应的name  表结构id,name
@finished time :2016 11 4 10:11
@attention:在查询type=1时候的，存在一个vid对应多个name，删除这个vid
"""
import zipfile
import os
import re
...
def mglive_vid_data_get(year,month,day):
    try:
        ...
        sql="select vid from mglive_hour_fact where type=1 and year=%s and month=%s and day=%s group by vid;"%(year,month,day)
        print "start to get mglive_vid_data_get_type_1"
        try:
            vid_data = pd.read_sql(sql,conn)
            print "get mglive_vid_data_get_type_1 data success"
        except:
            vid_data = pd.DataFrame()
            print "get mglive_vid_data_get_type_1  data  fails"
...
def  name_get_from_max_user(vid_str):
       conn = MySQLdb.connect(host='101.200.97.131', user='max_user_ro', passwd='Max_user_ro', port=3306, db='max_user',charset='utf8')
       sql = "select id as vid,nickName as name from user where id in(%s) "%vid_str
       print "start to get name_get_type1"
       try:
            name_get_type1=pd.read_sql(sql,conn)
            print "get name_get_type1 success"
       except:
           name_get_type1=pd.DataFrame()
           print "get name_get_type1 failure"
       return name_get_type1
...
def write_checkinfo(message):
    ...
    if not os.path.exists(file_path):
        os.mkdir(file_path)
    file_name = '/root/hf/live_user/mglive_vid_name/mglive_vid_name_1_check.txt'  # 检验文本的名称
    f = open(file_name, 'a')
...
def insert_vid_name(conn,cur,vid_name_list,date):
   ...
   try:
        sql = 'insert into mglive_vid_name_1 values(' + ','.join(map(lambda o: "%s", range(0, 2))) + ')'
        cur.executemany(sql,vid_name_list)
        conn.commit()
   except MySQLdb.Error, e:
        insert_tag = 0  ###插入失败表示0
        error_insertlog_path = '/root/hf/live_user/mglive_vid_name/mglive_vid_name_1_error_'+date+".txt"  # 存放插入错误的日志信息
...
##更新type=1  当存在一个vid对应多个name删除此行vid
def update_mglive_vid_name_1(conn,cur):
    try:
        sql="delete from mglive_vid_name_1 where id in( select id from (select id,count(1) as cishu from mglive_vid_name_1 group by id) as b where b.cishu>1);"
        cur.execute(sql)
        conn.commit()
        print "update mglive_vid_name_1 success"
    except MySQLdb.Error, e:
        print "update mglive_vid_name_1 falis"
        print "Mysql Error %d: %s" % (e.args[ 0 ], e.args[ 1 ])
...
if __name__ == '__main__':
    ...
    try:
        ...
        if length>0:
            ...
            ##获取vid对应的name
            name_get=name_get_from_max_user(vid_str)
            ##获取数据的行数
            ...
            if name_get_length>0:
               ...
               print info
               ##更新mglive_vid_name_1保证vid的唯一性
               update_mglive_vid_name_1(conn,cur)
            else:
